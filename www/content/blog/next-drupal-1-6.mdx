...
The new implementation is built on top of Drupal's Events API and `Revalidator` plugins.

You can now provide your own `Revalidator` plugins to execute custom code when an entity is inserted, updated or deleted.
...
```php title=custom_module/src/EventSubscriber/EventSubscriber.php
<?php
...
Bumping a minor in `next-drupal` will also bump same in `next` module.
