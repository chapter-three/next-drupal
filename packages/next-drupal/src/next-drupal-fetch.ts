...
export class NextDrupalFetch {
  accessToken?: NextDrupalFetchOptions["accessToken"]

  baseUrl: BaseUrl

  fetcher?: NextDrupalFetchOptions["fetcher"]
  ...
  /**
   * Instantiates a new NextDrupalFetch.
   *
   * const client = new NextDrupalFetch(baseUrl)
   *
   * @param {baseUrl} baseUrl The baseUrl of your Drupal site. Do not add the /jsonapi suffix.
   * @param {options} options Options for NextDrupalFetch.
   */
  ...
  async throwIfJsonErrors(response: Response) {
    if (!response?.ok) {
