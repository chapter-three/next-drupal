...
import { NextDrupal, NextDrupalFetch } from "../../src"
...
describe("baseUrl parameter", () => {
  ...
  test("returns a NextDrupal", () => {
    ...
    expect(new NextDrupal(BASE_URL)).toBeInstanceOf(NextDrupalFetch)
  })
})
...
describe("options parameter", () => {
  ...
  describe("useDefaultResourceTypeEntry", () => {
    test("defaults to `false`", () => {
      const drupal = new NextDrupal(BASE_URL)
      expect(drupal.useDefaultResourceTypeEntry).toBe(false)
    })
    ...
    test("can be set to `true`", () => {
      ...
      expect(drupal.useDefaultResourceTypeEntry).toBe(true)
    })
  })
})
