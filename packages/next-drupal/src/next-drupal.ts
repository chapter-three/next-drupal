...
export class NextDrupal extends NextDrupalBase {
  ...
  async getResourceByPath<T extends JsonApiResource>(
    ...
    await this.throwIfJsonErrors(
      response,
      "Error while fetching resource by path: ",
      response.status !== 207
    )
    ...
    if (!json?.["resolvedResource#uri{0}"]?.body) {
      const message = json?.router?.body
        ? JSON.parse(json.router.body)?.message
        : /* c8 ignore next */ "Error while translating path"

      this.logOrThrowError(new Error(message))
