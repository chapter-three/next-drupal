...
Here's a list of tasks we will accomplish in this guide:

1. Install and configure the Next.js module
...
Note: we are pulling the latest dev version since there are no stable releases for now.

<Callout>

#### Apply patch

Under the hood, the Next.js module uses [Subrequests](https://www.drupal.org/project/subrequests) to resolve entities in [`getStaticProps`](https://nextjs.org/docs/basic-features/data-fetching#getstaticprops-static-generation).
...
1. Open `composer.json` file and add the following lines (see highlighed) under `"extra"`.
...
Visit `/admin/modules` and enable the **Next.js** and the **Next.js JSON API** modules.
...
3. Fill in the following fields: **Label**, **Base URL**, **Preview URL**, **Preview Secret**.
4. Click **Save**
...
The `next-drupal` plugin uses paths to resolve entities for `getStaticProps`. To make this work, we need to configure path aliases for our content types.
...
- _Bypass content access control_
- Issue subrequests
...
We are going to use the [basic starter](https://github.com/arshad/next-drupal/tree/master/starters/basic-starter).
...
**Hint:** Since we're not using the Drupal front end theme, set **Seven** as the default theme for a cleaner preview UI.
